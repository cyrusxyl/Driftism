...
//  Driftism
//
//  Created by Cyrus Liu on 3/14/17.
...
open class AnalogJoystick: SKNode {
    ...
    open override func touchesMoved(_ touches: Set<UITouch>, with event: UIEvent?) {
        ...
        for touch: AnyObject in touches {
            ...
            let relativePosition = CGPoint(x:needPosition.x/maxDistantion, y:needPosition.y/maxDistantion)
            
            data = AnalogJoystickData(velocity: relativePosition, angular: -atan2(needPosition.x, needPosition.y))
        }
    }
